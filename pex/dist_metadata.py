...
class MetadataFiles(object):
    ...
    def has_metadata_file(self, metadata_file_name):
        # type: (Text) -> bool
        return self.metadata_file_rel_path(metadata_file_name) is not None
    ...
    class Value(Enum.Value):
        def iter_metadata_files(
            ...
            for metadata_files in iter_metadata_files(
                location, restrict_types_to=(self,), rescan=rescan
            ):
                yield metadata_files
...
def _is_dist_pkg_info_file_path(file_path):
    ...
    # TODO(John Sirois): XXX: Tighten this up any to ascii project name or other tightens?
    if not re.match("^.+-.+$", project_dir):
        return False
    ...
    for metadata_type in restrict_types_to or MetadataType.values():
        ...
        if key not in _METADATA_FILES:
            ...
            if MetadataType.DIST_INFO is metadata_type:
                if os.path.isdir(canonical_location):
                    listing.extend(
                        _find_installed_metadata_files(
                            canonical_location, "*.dist-info", "METADATA"
                        )
                    )
                ...
                listing.extend(
                    _find_installed_metadata_files(canonical_location, "*.egg-info", "PKG-INFO")
                )
...
class DistMetadata(object):
    ...
    def from_metadata_files(cls, metadata_files):
        ...
        return cls(
            ...
            requires_dists=tuple(requires_dists(metadata_files.metadata.pkg_info)),
            requires_python=requires_python(metadata_files.metadata.pkg_info),
        )
