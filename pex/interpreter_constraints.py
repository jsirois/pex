...
class InterpreterConstraint(object):
    ...
    @classmethod
    def compatible_version(cls, interpreter=None):
        ...
        return cls.parse(
            "{interpreter}~={version}".format(
                interpreter=python_identity.interpreter,
                version=".".join(map(str, python_identity.version[:2])),
            )
        )
...
class InterpreterConstraints(object):
    ...
    def parse(cls, constraints):
        # type: (Iterable[str]) -> InterpreterConstraints
        return cls(
            constraints=tuple(
